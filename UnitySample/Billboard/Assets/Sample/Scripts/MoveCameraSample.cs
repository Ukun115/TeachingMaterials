...
public class MoveCameraSample : MonoBehaviour
{
    /// <summary>
    /// 移動スピード
    /// </summary>
    ...
    void Update()
    {
        ...
        // 移動を実行
        var move = new Vector3(x, 0, z) * _moveSpeed * Time.deltaTime;
        transform.Translate(move);

        // 回転を実行（Y軸を中心に回転させる）
        // Vector3.up は (0, 1, 0) なので、水平に回る
        transform.Rotate(Vector3.up, rotate * _rotateSpeed * Time.deltaTime);
    }
}